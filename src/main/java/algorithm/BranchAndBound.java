...
public class BranchAndBound {
    ...
    public BranchAndBound(ArrayList<Billboard> billboardDatabase) {
    ...
    public void getSolution() {
        ...
        int iter = 0;
        while (lowerInf < upperInf && queue.size() > 0) {
            tuple = queue.poll();
            ...
            if (tuple.splitable()) {
                for (int i = 0; i < tuple.getCandidateSet().size(); i++) {
                    ...
                    computeBound = new ComputeBound(billboardSet_a, candidateSet);
                    computeBound.run();
                    ...
                    computeBound = new ComputeBound(billboardSet_b, candidateSet);
                    computeBound.run();
    ...
    private void checkResult(ComputeBound computeBound, Tuple tuple, int i, ArrayList<Billboard> billboardSet) {
